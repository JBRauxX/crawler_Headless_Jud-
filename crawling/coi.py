...
sheet = gc.open_by_key(SPREADSHEET_KEY).worksheet("COI")
...
def update_status_and_cekindo():
    ...
    while True:
        ...
        sheet.update_cell(row, 3, status)  # Status pada kolom C
...
def get_penanda_row():
    """
    Temukan baris yang berisi penanda untuk 'TANGGAL PENGECEKAN TERAKHIR :	'.
    """
    all_values = sheet.get_all_values()
    for idx, row in enumerate(all_values, start=1):
        if "TANGGAL PENGECEKAN TERAKHIR :	" in row:
            return idx
    return -1

def update_check_time_and_date():
    """
    Perbarui waktu pemeriksaan terakhir di Google Sheet di kolom khusus.
    """
    now = datetime.now()
    today = now.strftime("%y-%m-%d")
    current_time = now.strftime("%H:%M")

    # Menemukan baris dengan penanda dan menggeser 8 baris ke atas
    penanda_row = get_penanda_row()
    if penanda_row == -1:
        print("Penanda 'TANGGAL PENGECEKAN TERAKHIR :	' tidak ditemukan.")
        return
    
    update_row = penanda_row - 8  # Baris tempat waktu akan diperbarui
    if update_row > 0:
        # Pastikan kolom berbeda, misalnya kolom F
        sheet.update_cell(update_row, 6, today)      # Kolom F untuk tanggal
        sheet.update_cell(update_row + 1, 6, current_time)  # Kolom F untuk waktu
        print(f"Waktu pemeriksaan diperbarui di baris {update_row}: {today} {current_time}")
    else:
        print("Baris untuk pembaruan tidak valid.")
