...
def is_isp_block_page(html_content):
    ...
    # Kata kunci yang mengindikasikan halaman blokir
    block_keywords = ['internetpositif', 'internetbaik', 'trustpositif']
    ...
    # Periksa iframe src atau link
    iframes = soup.find_all('iframe', src=True)
    links = soup.find_all('a', href=True)
    for tag in iframes + links:
        url = tag.get('src') or tag.get('href')
        if any(keyword in url.lower() for keyword in block_keywords):
            return True
